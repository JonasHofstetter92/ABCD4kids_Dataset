...
#...................................................................................................#
#### ABCD4kids study: Extract Analysis Dataset from 4C and 3H database                           ####
...
study_group_4C <- only_pat_with_rrtdate_all_rows %>% filter(month %in% c(0, 12, 24, 36, 48, 60, 72, 84, 96, 108) & 
                                                ...
                                                select(patid, subid, gender, datadate, month, vdate, rrtdate, KRT_onset_by, diadate, tpldate,
                                                       ...
                                                       imtdate, imtl, imtr, imtcalcl, imtcalcr, imtsds,
                                                       pwvdate, pwvsys, pwvsyssds, pwvdia, pwvdiasds, 
                                                       pwv1, pwv2, pwv3, pwv, 
                                                       pwadate,
...
study_group_4C <- left_join(study_group_4C,  
                            select(cv_sds, patid, vdate, pwv_sds_a, pwv_sds_h, beta_sds_a, beta_sds_h, dc_sds_a, dc_sds_h, einc_sds_a, einc_sds_h), 
...
ABCD4kids_4C_analysis_DF <- rbind(study_group_DF, comparator_group1_DF, comparator_group2_DF) %>%
                              mutate(database = "4C") %>% select(patid, database,  ABCD4kids_group, subid, gender, country, everything())
...
ABCD4kids_4C_analysis_DF$hdmode <- as.character(factor(ABCD4kids_4C_analysis_DF$hdmode,
                                                       levels = c(1,2,3),
                                                       labels = c("HD", "HDF", "HF")))
...
data_3H <- data_3H %>% select(-c(lvmi)) %>% mutate(dt_basevis_copy = dt_basevis,
...
# round numeric variables up to desired number of decimal places
ABCD4kids_final_analysis_DF$heightsds <- round(ABCD4kids_final_analysis_DF$heightsds, 2)
ABCD4kids_final_analysis_DF$bmisds <- round(ABCD4kids_final_analysis_DF$bmisds, 2)
ABCD4kids_final_analysis_DF$imtl <- round(ABCD4kids_final_analysis_DF$imtl, 2)
ABCD4kids_final_analysis_DF$imtsds <- round(ABCD4kids_final_analysis_DF$imtsds, 5)
ABCD4kids_final_analysis_DF$lvmi <- round(ABCD4kids_final_analysis_DF$lvmi, 4)
ABCD4kids_final_analysis_DF$age <- round(ABCD4kids_final_analysis_DF$age, 2)

# use label format for diagnose2
ABCD4kids_final_analysis_DF$diagnose2 <- as.character(factor(ABCD4kids_final_analysis_DF$diagnose2,
                                                             levels = c(1,2,3,4,5),
                                                             labels = c("CAKUT",
                                                                        "Glomerulopathies",
                                                                        "Nephloss",
...
# reorder columns
ABCD4kids_final_analysis_DF <- ABCD4kids_final_analysis_DF %>% select(patid, 
                                                                      database,
                                                                      ABCD4kids_group,
                                                                      subid,
                                                                      gender,
